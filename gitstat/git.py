...
# '''
	def auth_all_user_repos_full(self):
		...
		repos_resp = requests.get("https://api.github.com/user/repos", headers=self.header)
...
# '''
	def auth_total_commit_count(self):
		...
		for repo in repos:
			commit_resp = requests.get("https://api.github.com/repos/" + repo + "/stats/contributors")
			commit_json = json.loads(commit_resp.text)
	...
	def auth_latest_commit_message(self, full_repo_name):

		repo_resp = requests.get("https:://api.github.com/repos/" + full_repo_name + "/commits", headers=self.header)
		json_dict = json.loads(repo_resp.text)
